...
public class Vuforia extends Unit {
    // IMPORTANT: If you are using a USB WebCam, you must select CAMERA_CHOICE = BACK; and PHONE_IS_PORTRAIT = false;
    private static final VuforiaLocalizer.CameraDirection CAMERA_CHOICE = BACK;
    private static final boolean PHONE_IS_PORTRAIT = false  ;

    private VuforiaLocalizer vuforia = config().vuforia;
    ...
    public Vuforia(){
        loading_establishing();
    }
    /**
     * 装载模型并建立坐标系
     */
    public void loading_establishing() {
     ...
     * 获取图案名称
     */
    String getResultInName(){
        String result = "none";
        targetVisible = false;
        for (VuforiaTrackable trackable : allTrackables) {
            if (((VuforiaTrackableDefaultListener)trackable.getListener()).isVisible()) {
//                telemetry().addData("Visible Target", trackable.getName())
                result = trackable.getName();
                targetVisible = true;
     ...
     * 获取位置
     */
    public double[] getResultInPosition(){
        targetVisible = false;
        for (VuforiaTrackable trackable : allTrackables) {
            if (((VuforiaTrackableDefaultListener)trackable.getListener()).isVisible()) {
//                telemetry().addData("Visible Target", trackable.getName())
...
//            telemetry().addData("Pos (in)", "{X, Y, Z} = %.1f, %.1f, %.1f", translation.get(0) / mmPerInch, translation.get(1) / mmPerInch, translation.get(2) / mmPerInch)
            return new double[]{translation.get(0) / mmPerInch,
...
//            telemetry().addData("Pos (in)", "{X, Y, Z} = %.1f, %.1f, %.1f", translation.get(0) / mmPerInch, translation.get(1) / mmPerInch, translation.get(2) / mmPerInch)
            ...
            return new double[]{rotation.firstAngle, rotation.secondAngle, rotation.thirdAngle};
